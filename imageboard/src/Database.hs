...
data ThreadPostRow = ThreadPostRow
  ...
  , tprPostText :: T.Text
  , tprPostFilename :: Maybe FilePath
...
getThreads h = do
  rows <-
    SQLite.query_
      (hConn h)
      "SELECT t.id, t.subject, p.id, p.thread_id, p.text, p.filename, p.created_at \
      \FROM threads t \
...
createPost h CreatePost{..} = do
  SQLite.execute
    (hConn h)
    "INSERT INTO posts (thread_id, text, filename) values (?,?,?)"
    (cpThreadId, cpText, cpFilename)
  SQLite.lastInsertRowId (hConn h)
