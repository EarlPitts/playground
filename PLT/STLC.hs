...
data Term =
  ...
  | TmAbs String Ty Term -- Stores the name of the variable for printing
  | TmApp Term Term
  deriving (Eq, Show)

type Context = [(String, Binding)] -- (Variable name, Binding)
...
getTypeFromContext ctx i =
  case getbinding ctx i of
    VarBind tyT -> tyT
    _           -> error "Wrong kind of binding for variable" ++ indexToName ctx i
...
typeOf ctx (TmIf t1 t2 t3) =
  if typeOf ctx t1 == TyBool then
    ...
    else error "Guard of conditional is not a boolean"
